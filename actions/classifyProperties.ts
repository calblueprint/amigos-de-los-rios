...
export type ClassifyResult =
  | { success: true; counts: { A: number; B: number; C: number }; updated: number }
...
export async function classifyProperties(
  ...
  if (propError) {
    return { success: false, error: `Failed to fetch properties: ${propError.message}` };
  }
  ...
  if (hydrantError) {
    return { success: false, error: `Failed to fetch hydrants: ${hydrantError.message}` };
  }
